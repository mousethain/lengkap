...
export Install_Limited=$( curl -s https://${Server_URL}/limit.txt | grep -w $License_Key | cut -d ' ' -f 2)
...
export Nama_Issued_License=$( curl -s https://${Server_URL}/limit.txt | grep -w $License_Key | cut -d ' ' -f 7| tr -d '\r' | tr -d '\r\n')
...
export Install_Limit=$( curl -s https://${Server_URL}/limit.txt | grep -w $License_Key | cut -d ' ' -f 2 | tr -d '\r' | tr -d '\r\n')
export Tipe_License=$( curl -s https://${Server_URL}/limit.txt | grep -w $License_Key | cut -d ' ' -f 8 | tr -d '\r' | tr -d '\r\n')
